...
function TrendLine({
  ...
  return (
    <svg ref={svgRef} viewBox={`0 0 ${width} ${height}`} className={className}>
...
function ObservabilityRuntimeView({ backHref = "/dashboard" }: { backHref?: string }) {
  ...
  const [maskIpEnabled, setMaskIpEnabled] = useState<boolean>(true);
  ...
  useEffect(() => {
    if (!data) return;
    setHistory((prev) => {
      const next = [...prev, { ts: Date.now(), payload: data }];
  ...
  useEffect(() => {
    ...
    const nextSource = urlSource === "all" || sources.includes(urlSource) ? urlSource : "all";

    if (nextSource !== sourceFilter) {
      setSourceFilter(nextSource);
    }

    if (urlSource !== nextSource) {
  ...
  }, [searchParams, sources, sourceFilter, pathname, router]);
  ...
  const snapshotAgeMs = data?.generatedAt ? Math.max(0, Date.now() - new Date(data.generatedAt).getTime()) : null;
      ...
      <section className="grid gap-4 md:grid-cols-4">
        {!data ? (
          Array.from({ length: 4 }, (_, i) => (
            <div key={i} className="rounded-xl border border-white/10 bg-black/20 p-4 animate-pulse">
      ...
      <section className="rounded-3xl border border-[#C9EB55]/18 bg-white/[0.03] p-6">
        ...
        <div className="space-y-2">
          {alerts.length > 0 ? alerts.map((alert) => (
            <div key={`${alert.severity}-${alert.message}`} className={`rounded-xl border px-4 py-3 text-sm ${severityClass[alert.severity]}`}>
