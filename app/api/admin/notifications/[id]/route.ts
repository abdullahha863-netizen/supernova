...
export async function PATCH(
  ...
  const rl = rateLimit(`${getClientIp(req)}:admin-notifications-patch`, { windowMs: 60_000, max: 30 });
  ...
  try {
    ...
    return NextResponse.json({ ok: true, notification, unreadCount });
  } catch (error) {
    if (error instanceof Error && "code" in error && error.code === "P2025") {
      return NextResponse.json({ ok: false, error: "Notification not found." }, { status: 404 });
    }

    console.error("[admin/notifications/:id][PATCH]", error);
